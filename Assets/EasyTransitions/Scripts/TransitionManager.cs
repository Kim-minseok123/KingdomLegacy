...
namespace EasyTransition
{
    ...
    {        
        ...
        public void Transition(TransitionSettings transition, float startDelay , Action action)
        {
            if (transition == null || runningTransition)
            {
                Debug.LogError("You have to assing a transition.");
            ...
            runningTransition = true;
            //NextAction = action;
            StartCoroutine(Timer(startDelay, transition, action));
        }
        /// <summary>
        ...
        IEnumerator Timer(float delay, TransitionSettings transitionSettings, Action action)
        {
            yield return new WaitForSecondsRealtime(delay);

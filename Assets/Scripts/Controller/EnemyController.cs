...
public class EnemyController : UI_Base
{
    ...
    {
        tempDamage = 0;
        Buff buff = buffList.GetBuffName("힘");
        if(buff != null)
            Damage += buffList.GetBuffName("힘").Value;
    ...
    public void GetVulenrable(int value) {
        ...
        if (buff == null)
        {
            if (buffList.Count > 6)
                return;
            buffList.Add(new Buff { Name = "취약", Value = value, controller = this, des = Define.Vulenerable});
        }
        else {
            buff.Value += value;
        }
    ...
    public void GetWeakness(int value) {
        Buff buff = buffList.GetBuffName("약화");
        if (buff == null)
        {
            if (buffList.Count > 6)
                return;
            buffList.Add(new Buff { Name = "약화", Value = value, controller = this, des = Define.Weakness });
        }
        else
        {
            buff.Value += value;
    ...
    {
        Buff buff = buffList.GetBuffName("민첩");
        if(buff != null)
    ...
    public void GetPower(int value)
    {
        ...
        if (buff == null)
        {
            if (buffList.Count > 6)
                return;
            buffList.Add(new Buff { Name = "힘", Value = value, controller = this, des = Define.Power });
        }
        else
        {
            buff.Value += value;
    ...
    {
        Buff buff = buffList.GetBuffName("힘감소");
        if (buff == null)
        {
            if (buffList.Count > 6)
                return;
            buffList.Add(new DePowerBuff { Name = "힘감소", Value = value, controller = this , des = Define.dePower});
        }
        else
        {
            buff.Value += value;
    ...
    public void GetAgility(int value)
    {
        ...
        if (buff == null)
        {
            if (buffList.Count > 6)
                return;
            buffList.Add(new Buff { Name = "민첩", Value = value, controller = this , des = Define.Agility});
        }
        else
        {
            buff.Value += value;
